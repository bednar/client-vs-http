...
func main() {
	...
	measurementName := flag.String("measurementName", fmt.Sprintf("sensor_%d", time.Now().UnixNano()), "destination")
	flag.Parse()
	...
	for i := 1; i <= *threadsCount; i++ {
		go doLoad(&wg, i, *measurementName, *secondsCount, *lineProtocolsCount, *influx)
	}
	...
	fmt.Println("-> rate [%]:        ", (total/expected)*100)
...
func doLoad(wg *sync.WaitGroup, id int, measurementName string, secondsCount int, lineProtocolsCount int, influx influxdb.Client) {
	defer wg.Done()
